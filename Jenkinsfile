pipeline 
{
    ...
    steps {
        ...
        def exitCode = sh(script: "docker run --name apitesting${BUILD_NUMBER} -e MAVEN_OPTS='-Dsurefire.suiteXmlFiles=src/test/resources/testrunners/testng_regression.xml' sanjaysagar/apitest:latest", returnStatus: true)
            if (exitCode != 0) {
